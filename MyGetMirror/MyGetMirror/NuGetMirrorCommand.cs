...
namespace MyGetMirror
{
    public class NuGetMirrorCommand
    {
        ...
        private const string SearchTaskName = "SEARCH";
        ...
        public NuGetMirrorCommand(int maxDegreeOfParallelism, NuGetPackageEnumerator enumerator, NuGetPackageMirrorCommand mirror, ILogger logger)
        {
            ...
            _taskNameWidth = Math.Max(minimumNameWidth, SearchTaskName.Length);
        }
        ...
        private async Task PopulatePackageIdentitiesAsync(ConcurrentBag<PackageIdentity> packageIdentities, ManualResetEventSlim isFullyEnumerated, CancellationToken token)
        {
            ...
            var total = 0;
            var searchTaskName = GetSearchTaskName();
            ...
            while (result.HasMoreResults)
            {
                ...
                _logger.LogInformationSummary($"[ {searchTaskName} ] A page of {result.PackageIdentities.Count} package(s) has been fetched.");
            ...
            _logger.LogInformationSummary($"[ {searchTaskName} ] The packages have been fully enumerated. {total} package(s) in total.");
        }

        private string GetSearchTaskName()
        {
            var searchTaskName = SearchTaskName.PadLeft(_taskNameWidth, ' ');
