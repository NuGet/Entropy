...
try
{
    ...
    var versionsNeedingSymbols = new ConcurrentDictionary<NuGetVersion, bool>();

    foreach (var package in allPackages)
    {
        foreach (var version in package.Value)
        {
            if (versionsNeedingSymbols.ContainsKey(version))
            {
                continue;
            }
            ...
            foreach (var dll in dlls)
            {
                using (var file = File.OpenRead(dll))
                {
                    ...
                    if (symbolMissing)
                    {
                        versionsNeedingSymbols.AddOrUpdate(version, true, (_, _) => true);
                        break;
                    }
                }
            }
            ...
            if (!symbolMissing)
            {
                Console.WriteLine("All symbols for {0} version {1} found!", package.Key, version);
            }
        }
    }

    if (versionsNeedingSymbols.Count > 0)
    {
        Console.WriteLine("pdbs need to be published for NuGet versions:");
        foreach (var version in versionsNeedingSymbols.Keys)
        {
    ...
    {
        Console.WriteLine("All pdbs published for all packages!");
    }
}
catch (Exception e)
{
