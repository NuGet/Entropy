...
namespace PackageHelper.Replay
{
    static class GraphSerializer
    {
        ...
        public static void WriteToFile<TGraph, TNode>(
            string path,
            TGraph graph,
            Action<JsonTextWriter, TGraph> writeGraphProperties,
            Action<JsonTextWriter, TNode> writeNode)
        ...
        public static TGraph ReadFromFile<TGraph, TNode>(
            string path,
            TGraph graph,
            Action<JsonSerializer, JsonReader, TGraph> readGraphProperty,
            Func<JsonSerializer, JsonReader, List<int>, TNode> readNode)
            where TGraph : IGraph<TNode>
            where TNode : INode<TNode>
        {
                ...
                while (j.TokenType == JsonToken.PropertyName)
                {
                    switch ((string)j.Value)
                    {
                        case "n":
                            ...
                            break;
                        default:
                            readGraphProperty(serializer, j, graph);
                            break;
                    }
