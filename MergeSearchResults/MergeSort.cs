...
namespace Merge
{
    ...
    {
        static IEnumerable<T> CompareAndSwap<T>(this IEnumerable<T> xl, Func<T, T, int> compare)
        {
            if (!xl.Skip(1).Any())
            {
                return xl;
            }
            if (compare(xl.First(), xl.Skip(1).First()) < 0)
            {
                return xl;
            }
        ...
        public static IEnumerable<T> MergeSort<T>(this IEnumerable<T> xl, Func<T, T, int> compare)
        {
            var splits = xl.Split();
